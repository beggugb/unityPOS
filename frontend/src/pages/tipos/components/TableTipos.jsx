...
class TableTipos extends React.Component {  
constructor(props){
    super(props);
    this.state = {      
      nroPagina:12,
...
makeHttpRequestWithPage = (pageNumber,num) => {
    this.props.tipoData(pageNumber,num)
} 
...
handleDelete = () => {            
this.props.tipoDelete(this.state.deleteId);  
   this.setState({
    modalDelete: false,    
    deleteId:0
   })
...
toggleModal = (item) => {   
  ...
  this.props.tipoItem(est,item)
}

render() {    
  const { data, pagina, paginas, total} = this.props.tipos   
...
export default connect(mapStateToProps,mapDispatchToProps)(TableTipos);